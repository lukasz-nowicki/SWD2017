...
public class Generation {
	...
	public Generation(int population, int gen_size,double total_available_capacity) {
		super();
...
//	
	...
	public Gen firstAlgorithm(double[] bestGenCriteria){
		...
		for(int i=0;i<POPULATION;i++){
			...
			double resultValueTemp= Math.abs(genTemp.getValue()-bestGenCriteria[0])+Math.abs(genTemp.getWeight()-bestGenCriteria[1])+
...
//	}
	...
	public List<Gen> computeValueResult(List<Gen> workingList,double epsilon,double bestGenCriteria){
		...
		for(int i=0;i<workingList.size();i++){
			...
			if (genValue >= (1 - epsilon) * bestValueForCriteria) {
				resultValue = genValue;
				System.out.println("Zapisujê "+ gen);
	...
	public List secondAlgorithm(int combination,double [] bestGenCriteria){
    	Gen gen = null;
...
//		System.out.println("OdpowiedŸ dla etapu 2: ");
